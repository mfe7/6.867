...
# predict is a function of a data point
# values is a list of values to plot

def plotDecisionBoundary(X, Y, predict, values, title = ""):
    ...
    zz = array([predict(reshape(x, (1,x.shape[0]))) for x in c_[xx.ravel(), yy.ravel()]])
    zz = zz.reshape(xx.shape)
    #pl.figure()
    CS = pl.contour(xx, yy, zz, values, colors = 'green', linestyles = 'solid', linewidths = 2)
    pl.clabel(CS, fontsize=9, inline=1)
    # Plot the training points
    pl.scatter(X[:, 0], X[:, 1], c=(1.-Y), s=50, cmap = pl.cm.cool, linestyles='-')
    pl.title(title)
    pl.axis('tight')
    ...
    pl.xlabel('x [m]')
    pl.ylabel('y [m]')
    pl.title('cyan = cross, violet = no cross')
    pl.show()
